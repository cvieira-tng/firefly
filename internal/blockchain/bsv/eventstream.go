...
func (s *streamManager) getEventStreams(ctx context.Context) (streams []*eventStream, err error) {
	...
	if err != nil || !res.IsSuccess() {
		return nil, ffresty.WrapRestErr(ctx, res, err, coremsgs.MsgBsvconnectRESTErr)
	}
	return streams, nil
}
...
//nolint:unused
func (s *streamManager) getListener(ctx context.Context, streamID string, listenerID string, okNotFound bool) (l *listener, err error) {
	res, err := s.client.R().
		SetContext(ctx).
		SetResult(&l).
...
func (s *streamManager) createListener(ctx context.Context, streamID, name, lastEvent string, filters []filter) (l *listener, err error) {
...
func (s *streamManager) deleteListener(ctx context.Context, streamID, listenerID string) error {
	...
	if err != nil || !res.IsSuccess() {
		return ffresty.WrapRestErr(ctx, res, err, coremsgs.MsgBsvconnectRESTErr)
	}
	return nil
}

func (s *streamManager) ensureFireFlyListener(ctx context.Context, namespace string, version int, firstEvent, streamID string) (l *listener, err error) {
	...
	// For BSV, we listen for BatchPin events
	filters := []filter{{
		eventfilter{
			EventType: "BatchPin",
		},
